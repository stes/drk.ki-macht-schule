...
function makeGUI() {
  ...
  let ownDataThumbnails = d3.selectAll("canvas[data-ownDataset]");
  ownDataThumbnails.on("click", function() {
    let newDataset = ownDatasets[this.dataset.owndataset];
    if (newDataset === state.ownDataset) {
      return; // No-op.
    }
    state.ownDataset =  newDataset;
    ownDataThumbnails.classed("selected", false);
    d3.select(this).classed("selected", true);
    generateData();
    parametersChanged = true;
    reset();
  ...
  let ownDatasetKey = getKeyFromValue(ownDatasets, state.ownDataset);
  // Select the dataset according to the current state.
  d3.select(`canvas[data-ownDataset=${ownDatasetKey}]`)
    .classed("selected", true);
...
function getAcc(network: nn.Node[][], dataPoints: Example2D[]): number {
  let loss = 0;
  for (let i = 0; i < dataPoints.length; i++) {
    let dataPoint = dataPoints[i];
    let input = constructInput(dataPoint.x, dataPoint.y);
    let output = nn.forwardProp(network, input);
    loss += nn.Errors.ACC.error(output, dataPoint.label);
  }
  return loss / dataPoints.length;
}
...
function updateUI(firstStep = false) {
  ...
  d3.select("#acc-test").text(humanReadable(accTest));
...
function oneStep(): void {
  ...
  accTrain = getAcc(network, trainData);
  accTest = getAcc(network, testData)
  updateUI();
}
...
function drawDatasetThumbnails() {
  ...
  if (state.problem === Problem.REAL_CLASSIFICATION) {
    for (let ownDataset in ownDatasets) {
      console.log(ownDataset);
      let canvas: any =
          document.querySelector(`canvas[data-ownDataset=${ownDataset}]`);
      let dataGenerator = ownDatasets[ownDataset];
      renderThumbnail(canvas, dataGenerator);
    }
  }
}
...
function generateData(firstTime = false) {
  ...
  else if (state.problem == Problem.REAL_CLASSIFICATION) {
    numSamples = NUM_SAMPLES_CLASSIFY;
    generator = state.ownDataset;
  }
