...
function createHistogram(id, model, noThreshold, includeAnnotation) {
  ...
  var thresholdLabel = svg.append('text').text('Entscheidungsgrenze: 50')
      .attr('x', tx)
      .attr('y', 40)
  ...
  function setThreshold(t, eventFromUser) {
    ...
    thresholdLabel.attr('x', labelX).text('Entscheidungsgrenze: ' + t);
    if (includeAnnotation) {
...
function createHistogramLegend(id, category) {
  ...
  var textPad = 4;
  svg.append('text')
      .text('nicht behandelt / erkrankt')
      .attr('x', centerX - boxSide - textPad)
      .attr('y', 2 * boxSide - textPad)
      .attr('text-anchor', 'end')
      .attr('class', 'legend-label');
  svg.append('text')
      .text('nicht behandelt / gesund')
      .attr('x', centerX - boxSide - textPad)
      .attr('y', boxSide - textPad)
      .attr('text-anchor', 'end')
      .attr('class', 'legend-label');
  svg.append('text')
      .text('behandelt / erkrankt')
      .attr('x', centerX + boxSide + textPad)
      .attr('y', 2 * boxSide - textPad)
      .attr('text-anchor', 'start')
      .attr('class', 'legend-label');
  svg.append('text')
      .text('behandelt / nicht erkrankt')
      .attr('x', centerX + boxSide + textPad)
      .attr('y', boxSide - textPad)
      .attr('text-anchor', 'start')
      .attr('class', 'legend-label');
...
function createSimpleHistogramLegend(id, category) {
  ...
  var textPad = 4;
  svg.append('text')
      .text('Tier ist erkrankt')
      .attr('x', centerX + boxSide + textPad)
      .attr('y', boxSide - textPad)
      .attr('text-anchor', 'start')
      .attr('class', 'legend-label');
  svg.append('text')
      .text('Tier ist gesund')
...
function createRatePies(id, model, palette, includeAnnotations) {
  ...
  explanation(svg, ['Anteil kranker Hunde,',
     'die behandelt werden'], 0, topY);
  explanation(svg, ['Anteil aller Hunde,',
     'die behandelt werden'], width / 2 + 4, topY);
...
function createCorrectnessMatrix(id, model) {
  ...
  var topY = 18;
  var correctLabel = createPercentLabel(svg, 0, topY, 'Korrekte Entscheidung',
      'pie-label', 'pie-number');
  var incorrectLabel = createPercentLabel(svg, width / 2 + 4, topY, 'Falsche Entscheidung',
      'pie-label', 'pie-number');

  // Add explanation of correct decisions.
  explanation(svg, [
      'krank & behandelt und', 'gesund & nicht behandelt'], 0, topY);
  explanation(svg, ['gesund & behandelt und',
      'krank & nicht behandelt'], width / 2 + 4, topY);
